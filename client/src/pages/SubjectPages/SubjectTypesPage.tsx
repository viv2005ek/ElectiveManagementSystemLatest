...
import { SubjectType } from "../../hooks/subjectTypeHooks/useSubjectTypes";
import { PlusIcon } from "@heroicons/react/24/outline";
import { Link } from "react-router-dom";
import Skeleton from "react-loading-skeleton";
import "react-loading-skeleton/dist/skeleton.css";
import useSubjectTypes from "../../hooks/subjectTypeHooks/useSubjectTypes.ts";

const SubjectTypesPage = () => {
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [selectedSubjectType, setSelectedSubjectType] =
    useState<SubjectType | null>(null);
  const { subjectTypes, fetchSubjectTypes, deleteSubjectType, error, loading } =
    useSubjectTypes();
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {loading ? (
                      ...
                      <tr>
                        <td
                          colSpan={3}
                          className="px-3 py-4 text-sm text-red-500 text-center"
                        >
                      ...
                      subjectTypes.map((subjectType: SubjectType) => (
                        <tr key={subjectType.id}>
                          ...
                          <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                            <Link
                              to={`/subject-types/${subjectType.id}/edit`}
              ...
              <div>
                ...
                <div className="mt-2">
                  <p className="text-sm text-gray-500">
                    ...
                    <strong>{selectedSubjectType?.name}</strong>? This action
                    cannot be undone.
                  </p>
                </div>
              </div>
