...
export default function CourseBucketsTable({
  ...
  isLoading: boolean;
  setSelectedBuckets?: Dispatch<SetStateAction<CourseBucket[]>>;
  ...
  const toggleBucketSelection = (e: MouseEvent, bucket: CourseBucket) => {
    e.stopPropagation();
    if (!setSelectedBuckets) return;

    const isSelected = selectedBuckets.some((b) => b.id === bucket.id);
    
    if (isSelected) {
      // Remove from selection
      setSelectedBuckets(selectedBuckets.filter((b) => b.id !== bucket.id));
    } else {
      // Add to selection
  ...
  const isBucketSelected = (bucket: CourseBucket) => {
    return selectedBuckets.some((b) => b.id === bucket.id);
  };
                    ...
                    <React.Fragment key={bucket.id}>
                      ...
                      >
                        ...
                        <td className="py-4 px-4 text-sm">
                          <button
                            onClick={(e) => toggleBucketSelection(e, bucket)}
                            className={`h-5 w-5 rounded border-2 flex items-center justify-center transition-colors duration-150 ${
                              isSelected
