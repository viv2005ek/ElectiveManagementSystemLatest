export const getBatches = (numberOfPastBatches: number, numberOfFutureBatches: number) => {
  const year = new Date().getFullYear();
  return Array.from({ length: numberOfPastBatches + numberOfFutureBatches + 1 }, (_, i) => ({
