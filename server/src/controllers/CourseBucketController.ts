...
const CourseBucketsController = {
  getAllCourseBuckets: async (req: Request, res: Response): Promise<any> => {
    try {
      const { departmentId, schoolId, facultyId, subjectTypeId } = req.query;

      const courseBuckets = await prisma.courseBucket.findMany({
        where: {
          isDeleted: false,
          ...(departmentId ? { departmentId: String(departmentId) } : {}),
          ...(schoolId ? { department: { schoolId: String(schoolId) } } : {}),
          ...(facultyId
            ? { department: { school: { facultyId: String(facultyId) } } }
            : {}),
          ...(subjectTypeId
            ? { subjectTypes: { some: { id: String(subjectTypeId) } } }
            : {}),
        },
        include: {
          department: {
        ...
        courseBuckets.map((bucket) => {
          ...
          return prisma.courseBucket.create({
            data: {
              ...
              subjectTypes: bucket.subjectTypeIds
                ? {
                    connect: bucket.subjectTypeIds.map((id: string) => ({
                      id,
                    })),
                  }
                : undefined,
